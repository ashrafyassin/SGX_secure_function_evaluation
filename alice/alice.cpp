...
/* IPC between enclaves with these files */
...
#define ALICE_PRIVATE_FILE "../alice_prv_key"
...
/********************************** 2. Initialize signing keys **********************************/
    ecall_status = ecall_init_signing_keys(global_eid, &ecall_return, ALICE_PRIVATE_FILE, ALICE_VERIFY_KEY_FILE, BOB_VERIFY_KEY_FILE);
...
/******************************* 3. Pass clients file to enclave *******************************/
    ...
    if (ecall_return != 0) {
        printf("processing alice clients file failed %d \n", ecall_return);
    }


/************************************* 4. Agree on DH keys *************************************/
    ecall_status = ecall_create_DH_keys(global_eid, &ecall_return,ALICE_DH_KEY_FILE, BOB_DH_KEY_FILE);
    if (ecall_status != SGX_SUCCESS)
        abort();
...
/************************** 5. Encrypt client data with the shared key **************************/
    ecall_status = ecall_encrypt_clients_data(global_eid, &ecall_return,ALICE_CLIENTS_FILE);
    if (ecall_status != SGX_SUCCESS)
        abort();

    if (ecall_return != 0) {
        printf("encrypting alice clients failed %d \n", ecall_return);
        return -1;
    }


/************************** 6. Pass the other side's encrypted clients **************************/
    ecall_status = ecall_decrypt_and_process_file(global_eid, &ecall_return,BOB_CLIENTS_FILE);
    if (ecall_status != SGX_SUCCESS)
        abort();
